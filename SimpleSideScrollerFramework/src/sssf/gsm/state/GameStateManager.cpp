...
void GameStateManager::loadLevel(Game *game, unsigned int initLevel)
{
	if ((initLevel > NO_LEVEL_LOADED) && (initLevel <= levelNames.size()))
	{
		...
		dataLoader->loadWorld(game, L"data/levels/SideScrollerMetal/", L"SideScrollerMetalLevel.tmx");
	}
}
...
/*
	...
	again.
*/
void GameStateManager::shutdown(Game *game)
{
...
*/
void GameStateManager::unloadCurrentLevel(Game *game)
{
	spriteManager->unloadSprites(game);
	world.unloadWorld();
}
