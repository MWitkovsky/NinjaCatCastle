...
static const int		LIVES_LABEL_X = 910;
static const int		LIVES_LABEL_Y = 700;
...
void BugginOutTextGenerator::appendLivesCounter(Game *game){
	wstringstream wss;
	wss << L"x ";
...
void BugginOutTextGenerator::initText(Game *game)
{
	...
	text->addText(&textToGenerate, LIVES_COUNTER_X, LIVES_COUNTER_Y, game->getGraphics()->getScreenWidth(), game->getGraphics()->getScreenHeight());
}
